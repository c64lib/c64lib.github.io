...
As it is a macro-hosted subroutine, it must be instantiated with screen and color RAM address first.
Such subroutine can be then to output different texts at different screen position and with different color.
